...
const SpaceAddress = ({ street_name, ward, district, province }: SpaceAddressProps) => {
    ...
    return (
        <Space>
            <Text type="secondary" style={{ fontSize: 12 }}>{t("Address")}:</Text>
            <Text>
                <a target={"_blank"} rel="noreferrer" href={formatUrlGoogleMap(street_name, ward, district, province)}>{street_name} - {ward} - {district} - {province}</a>
            </Text>
        </Space>
    )
}
...
const ContentCol = ({ label, value, isTagStatus = false, isTagFarmType = false }: ContentColProps) => {
        ...
        <Space>
            <Text type="secondary" style={{ fontSize: 12 }}>{t(label)}:</Text>
            {
                isTagStatus
                    ...
                    ? <Tag color={value === "Infected" ? "#ff4d4f" : "#4BB543"} key={value}> {t(value)} </Tag>

                    : isTagFarmType

                        ? <Tag color={value === "Sow" ? "volcano" : value === "Finishing" ? "green" : "geekblue"} key={value}> {t(value)} </Tag>
...
const ContentRowCreatedTime = ({ data }: ContentRowCreatedTimeProps) => (
    <Row gutter={[0, 10]}>
        ...
        {
            data?.infected === 1 && <Col span={24}>
                <SpaceAddress street_name={data?.street_name} ward={data?.ward} district={data?.district} province={data?.province} />
            </Col>
        }
    </Row>
)
...
const ContentRowUpdatedTime = ({ data }: ContentRowCreatedTimeProps) => (
    <Row gutter={[0, 10]}>
        ...
        <Col xs={24} md={12}>
            <ContentCol label="Updated Date" value={moment(data?.updated_time).format("DD-MM-YYYY") as string} />
        </Col>
        ...
        <Col span={24}>
            <ContentCol label="Duration" value={calDuration(moment(data?.updated_time).unix() - moment(data?.report_date).unix()) as string} />
        </Col>

        {
            data?.infected === 0 && <Col span={24}>
                <SpaceAddress street_name={data?.street_name} ward={data?.ward} district={data?.district} province={data?.province} />
            </Col>
        }
    </Row>
